...
def room(request, room_pk):
    ...
    response = {
        "roomname":chatroom.chatroom,
        "messages":ChatSerializer(messages, many=True).data
    }
...
def createChatroom(request):
    ...
    if requestuser.role == 'u':
        ...
        try:
            ...
            print('3')
            return Response({'detail':'chat already exists', 'room_id':chatroom.id, 'location':chatroom.chatroom}, status=status.HTTP_301_MOVED_PERMANENTLY)
        except ObjectDoesNotExist:
            chatroom = ChatroomList.objects.create(
                user=requestuser,
                driverorcompany=User.objects.get(id=request.data['driverorcompany']),
                chatroom=uuid.uuid4()
            )
            return Response({'detail':'chat created', 'room_id':chatroom.id, 'location':chatroom.chatroom}, status=status.HTTP_201_CREATED)
